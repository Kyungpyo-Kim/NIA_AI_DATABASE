...
class BagToCanCsv:
  def __init__(self, can_topic, output_dir):
    ...
    self.can_file = csv.writer(self.csv_file, delimiter=',',
      quotechar='|', quoting=csv.QUOTE_MINIMAL)

  def processMsg(self, topic, msg, t, week, time_of_week, utc):
    self.can_file.writerow(
      [self.seq, t.to_sec(),  msg.id, msg.is_rtr, msg.is_extended, msg.is_error, 
        msg.dlc, msg.data.encode("hex"), week, time_of_week, utc])
    self.seq  += 1

  def writeHeader(self):
    self.can_file.writerow(
      ['sequence', 'ros time [sec]', 'id', 'is rtr', 'is extended', 'is error', 
        'dlc', 'data[hex]', 'week', 'time of week [nsec]', 'utc'])
...
def main():
  """Extract a folder of can from a rosbag.
  """
  parser = argparse.ArgumentParser(
    description="Extract can from a ROS bag.")
