...
class BagToGpsCsv:
  def __init__(self, gps_topic, gps_time_topic, output_dir):
    ...
    self.gps_file = csv.writer(self.csv_file, delimiter=',',
  ...
  def processMsg(self, topic, msg, t, week, time_of_week, utc):
    ...
    if topic == self.gps_topic:
      self.gps_file.writerow(
        [self.seq, t.to_sec(),  msg.latitude, msg.longitude, msg.altitude, 
          msg.status.status, week, time_of_week, utc])
      self.seq  += 1

  def writeHeader(self):
    self.gps_file.writerow(
      ['sequence', 'ros time [sec]', 'latitude', 'longitude', 'altitude', 
        'status', 'week', 'time of week [nsec]', 'utc'])
...
def main():
  """Extract a folder of gps from a rosbag.
  """
  parser = argparse.ArgumentParser(
    description="Extract gps from a ROS bag.")
